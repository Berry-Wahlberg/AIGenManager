    ...
    private static (string Operation, string Query, IEnumerable<object> Bindings) ParseNode(NodeFilter node)
        {
            ...
            // Define property operation and prop variables early
            ...
            switch (comparison)
            {
                ...
                case NodeComparison.Equals:
                    ...
                    toper = "=";
                    break;
                case NodeComparison.GreaterThan:
                    toper = ">";
                    break;
                case NodeComparison.GreaterThanOrEqual:
                    toper = ">=";
                    break;
                case NodeComparison.LessThan:
                    toper = "<";
                    break;
                case NodeComparison.LessThanOrEqual:
                    toper = "<=";
                    break;
                case NodeComparison.Between:
                    // Split the value into two parts for range comparison
                    var values = value.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
                    if (values.Length == 2)
                    {
                        conditions.Add(new KeyValuePair<string, object>("(cmfyp.Value BETWEEN ? AND ?)", new object[] { values[0], values[1] }));
                        // Skip the default condition adding below
                        return (
                            operation.ToString(),
                            ...
                            $"AND ({string.Join(" AND ", conditions.Select(c => c.Key))})",
                    ...
                    // If invalid Between format, fallback to Equals
                    value = $"{value}";
                    toper = "=";
                    break;
            }
