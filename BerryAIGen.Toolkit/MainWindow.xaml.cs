    ...
    public partial class MainWindow : BorderlessWindow
    {
        ...
        public MainWindow()
        {
            ...
            try
            {
                ...
                Logger.Log($"Calling InitializeComponent");
                InitializeComponent();

                Logger.Log($"Registering GlobalExceptionHandler");
                AppDomain currentDomain = AppDomain.CurrentDomain;
                currentDomain.UnhandledException += new UnhandledExceptionEventHandler(GlobalExceptionHandler);

                Logger.Log($"Reading samplers.txt file");
                // Try to read samplers.txt, if it doesn't exist, use an empty list
                if (File.Exists("samplers.txt"))
                {
                    QueryBuilder.Samplers = File.ReadAllLines("samplers.txt").ToList();
                }
                else
                {
                    QueryBuilder.Samplers = new List<string>();
                    Logger.Log("Warning: samplers.txt file not found, using empty list");
                }
        ...
        private async void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            if (!_configuration.Exists())
            {
                ...
                // Show language selection window first
                var languageWindow = new LanguageSelectionWindow(_settings);
            ...
            await dataStore.Create(
                () => _messagePopupManager.ShowMessage("Please wait while we update your database", "Updating Database"),
                (handle) =>
                {
